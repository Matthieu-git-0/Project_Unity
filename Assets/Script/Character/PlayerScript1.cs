...
public class PlayerScript : MonoBehaviour, IPunObservable
{
    ...
    void Start()
    {
        ...
        //Debug.Log($"PhotonView ID: {view.ViewID}, Is Mine: {view.IsMine}");
    ...
    void Update()
    {
        ...
        if (view.IsMine)
        {
            if (time < Maxtime)
            {
                ...
                else // Sinon Update des param�tres de pause
                {
                    UdpateSensitivityCamera();
                }

                CheckPauseActivation(); // Activation ou d�sactivation du menu pause)
            }
        ...
        {
            // Application des param�tres synchronis�s pour les autres joueurs
    ...
    public void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
    {
        if (stream.IsWriting)
        {
            // Envoie des param�tres de l'Animator locaux aux autres joueurs
        ...
        {
            // Reception des param�tres de l'Animator depuis le serveur
            ...
            // Appliquer ces param�tres � l'Animator local
            animator.SetFloat("Sides", animatorSides);
            animator.SetFloat("Front/Back", animatorFrontBack);
            animator.SetBool("isRunning", animatorIsRunning);
            animator.SetBool("isJumping", animatorIsJumping);
    ...
    // Update des sensibilit�s de rotation camera
    ...
    // D�placements de camera
    private void MoveCamera()
    {
        rotationX += -Input.GetAxis("Mouse Y") * lookSensitivityX;												// Detection du mouvement Y de souris
        rotationX = Mathf.Clamp(rotationX, -lookXLimit, lookXLimit);											// Blocage du mouvement Y selon les param�tres pr�d�finis
        playerCamera.transform.localRotation = Quaternion.Euler(rotationX, 0, 0);								// D�placement de la camera selon les mouvements de la souris
    ...
    private void MovePlayer()
    {
        ...
        // Normalisation du d�placement diagonal
        ...
        {
            moveDirection.y -= gravity * Time.deltaTime; // Application de la gravit�
            animator.SetBool("isJumping", false);
        }
        ...
        // D�placement via CharacterController
    ...
    private void UpdateAnimatorParameters()
    {
        ...
        // V�rification des touches directionnelles
        ...
        // Mise � jour de l'Animator avec des entiers
        animator.SetFloat("Sides", moveX);
        animator.SetFloat("Front/Back", moveY);
        animator.SetBool("isRunning", isRunning);

        // Enregistrement pour la synchronisation r�seau
        animatorSides = moveX;
